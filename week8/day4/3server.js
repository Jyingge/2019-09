...
app.use(session({
    ...
    secret: 'ZFPX',//session会根据这个属性和后端重在session的属性名来生成对应的字段
...
app.post('/login', function (req, res) {
    ...
    if (bol) {
        ...
        req.session.username = username;//种东西，后端在session上种植了一个属性
        res.send({
            code: 0,
            data: {
                name: username
...
app.get('/info', function (req, res) {
    ...
    // 登录成功之后，后端给前端种植了cookie。以后每一次请求后台接口的时候，后端都会根据这个cookie值去判断前端是否处于有效 登录期内。
    // 那此时，后端的额具体写法是根据登录时在session上设置的属性，还有没有来进行判断
